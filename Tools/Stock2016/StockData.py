...
def RankOfRecent(index,dic):
	rankAbove = 0
	length = len(dic)
	closePrice = float(dic[index]["AdjClose"])
	for i in range(length - TradeDays,length):
		if i != index:
...
while(True):
	number = raw_input("Enter your choose(number):")
	isEqual = False
	for i in range(0,11):
		if(cmp(str(i),number) == 0):
			isEqual = True
			break;
	if(isEqual == False):
		break;
	if(isGet(number) == False):
		chooseArray.append(number)
